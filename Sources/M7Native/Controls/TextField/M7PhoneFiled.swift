...
//  
//
//  Created by Alexander Romanv on 14.10.2020.
//

import SwiftUI

public struct M7PhoneFiled: View {
    
    @Binding public var phoneNumber: String
    @State public var number: String = ""
    @State public var countryCode : String = ""
    @State public var countryFlag : String = ""
    @State public var showModal = false
    
    public init(_ phoneNumber: Binding<String> = .constant("")) {
        self._phoneNumber = phoneNumber
    }
    
    public let countryDictionary  = ["AF":"93","AL":"355","DZ":"213","US":"1",
    ...
    public var body: some View {
        ...
        M7Surface(elevation: .z3, padding: .zero) {
        
        ZStack {
            HStack (spacing: 0) {
                
                M7Button(style: .text, action: { showModal.toggle() }) {
    ...
    func currentCode() -> String {
        ...
        let code = countryDictionary[current ?? "US"]
...
struct M7PhoneFiled_Previews: PreviewProvider {
    
    static var previews: some View {
        M7PhoneFiled(.constant(""))
    }
}
